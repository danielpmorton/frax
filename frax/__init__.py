...
def check_env_vars():
    ...
    if (len(devices) == 1 and devices[0].platform == "cpu") or (
        "JAX_PLATFORMS" in os.environ and os.environ["JAX_PLATFORMS"] == "cpu"
    ):
        ...
        msg = (
            "[frax] CPU backend detected but some environment variables are not configured.\n"
        ...
        if not x64_enabled:
            msg += (
                "\n- JAX_ENABLE_X64 not detected.\nRecommendation: set JAX_ENABLE_X64=1"
            )
        ...
        should_warn = not x64_enabled or not single_thread_enabled
        if should_warn:
